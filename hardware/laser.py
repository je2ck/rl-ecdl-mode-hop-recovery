...
from .control import wait_for_change
...
def fetch_data_safe(dlc_controller, api_client):
    ...
    try:
        frequency_value = float(frequency_raw)
    except (TypeError, ValueError) as error:
        raise RuntimeError("Wavemeter returned an invalid frequency") from error
    frequency = frequency_value / 1e3 if frequency_value > 1e6 else frequency_value
...
class DiodeLaser:
    ...
    def emit(self, signal: Signal) -> tuple[LaserState, float]:
        ...
        if signal == Signal.from_str("CurrentUp"):
            for _ in range(repeat):
                ...
                wait_for_change(
                    self.dlc_controller.get_act_current, current_act, threshold=0.06
                )
            ...
            wait_for_change(
                self.dlc_controller.get_act_voltage, voltage_act, threshold=0.06
            )
    ...
    def fetch_data(self) -> LaserState:
        for _ in range(10):
            ...
            if obs_data["frequency"] >= 0:
                break
            time.sleep(0.1)
        else:
            raise RuntimeError("Wavemeter did not return a valid frequency")
