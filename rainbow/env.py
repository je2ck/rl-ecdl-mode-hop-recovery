...
from .interface import LaserInterface, IMAGE_SIZE
import random
import cv2
import torch
import numpy as np
...
from .interface import SENSOR_DIM, IMAGE_SIZE
...
class Env():
  def __init__(self, args):
    ...
    if args.simulation:
      self.ale = LaserInterface(args.target_frequency, args.current_range, args.random_target)
    else:
      ...
      plotter = PlotManager()
      self.ale = LaserInterface(args.target_frequency, args.current_range, args.random_target, api_client, self.dlc_controller, self.oscilloscope, is_test=True, plotter=plotter)
  ...
  def _get_state(self):
    img = cv2.resize(self.ale.getScreenGrayscale(), (IMAGE_SIZE, IMAGE_SIZE), interpolation=cv2.INTER_LINEAR)
    image_tensor = torch.tensor(img, dtype=torch.float32, device=self.device).div_(255)
    if self.image_only:
      return {"image": image_tensor}

    freq_diff, current, voltage, pzt_range, current_range, is_stable, action = self.ale.getSensorData()
    sensor_tensor = torch.tensor([freq_diff, current, voltage, pzt_range, current_range, is_stable, action], dtype=torch.float32, device=self.device)
    return {"image": image_tensor, "sensor": sensor_tensor}

  def _reset_buffer(self):
    self.state_buffer = deque([], maxlen=self.history_length)
    default_state = {
      "image": torch.zeros(IMAGE_SIZE, IMAGE_SIZE, device=self.device, dtype=torch.float32),
    }
    if not self.image_only:
      default_state["sensor"] = torch.zeros(SENSOR_DIM, device=self.device, dtype=torch.float32)
    for _ in range(self.window):
      self.state_buffer.append(default_state)

  def change_temperature(self, up: bool):
    curr_temp = self.dlc_controller.get_temp()
    print(f"[{datetime.now().strftime('%m%d_%H:%M:%S')}] Before: Temperature now {curr_temp} C")
    ...
    while abs(act_temp - target_temp) < 0.02:
      act_temp = self.dlc_controller.get_act_temp()
    print(f"[{datetime.now().strftime('%m%d_%H:%M:%S')}] After: Temperature now {target_temp} C")
    time.sleep(10)
  ...
  def render(self):
    ...
    if hasattr(self, 'gradcam'):
        ...
        cam_up = cv2.resize(cam_map, (512, 512),
        ...
        cv2.putText(overlay, f"Action: {action_idx}", (5,20),
