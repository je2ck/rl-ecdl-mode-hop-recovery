    ...
    def log_episode(self, episode_info):
        with self.lock:
            try:
                ...
                with open(
                    os.path.join(self.results_dir, "episodes_fallback.log"), "a"
                ) as f:
        ...
        with self.lock:
            try:
                ...
                temp_filepath = os.path.join(
                    self.results_dir, f"evaluation_T_{evaluation_step}_temp.json"
                )
                ...
                with open(
                    os.path.join(self.results_dir, "evaluation_fallback.log"), "a"
                ) as f:
...
class StreamingLogger:
    ...
    def __init__(self, results_dir):
        self.results_dir = results_dir
        self.episode_stream = open(
            os.path.join(results_dir, "episodes_stream.jsonl"), "a"
        )
        self.eval_stream = open(
            os.path.join(results_dir, "evaluation_stream.jsonl"), "a"
        )
        self.lock = threading.Lock()
