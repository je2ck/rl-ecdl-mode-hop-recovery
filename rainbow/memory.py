...
class ReplayMemory():
  ...
  def append(self, state, action, reward, terminal):
    if self.image_only:
      ...
      img = obs["image"].mul(255).to(dtype=torch.uint8, device=torch.device('cpu'))
      self.transitions.append((self.t, img, action, reward, not terminal), self.transitions.max)
      self.t = 0 if terminal else self.t + 1
    else:
      obs = {
          "image": state["image"][-1],
          "sensor": state["sensor"][-1]
      }
      img = obs["image"].mul(255).to(dtype=torch.uint8, device=torch.device('cpu'))
      sensor = obs["sensor"].to(dtype=torch.float32, device=torch.device('cpu'))
      self.transitions.append((self.t, img, sensor, action, reward, not terminal), self.transitions.max)
      self.t = 0 if terminal else self.t + 1
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    if not valid:
      raise RuntimeError("Unable to sample transitions with sufficient history/future after {} attempts.".format(attempts))

    transitions = self._get_transitions(idxs)
    if self.image_only:
      all_images = transitions['image']
      states_images = torch.tensor(all_images[:, :self.history], device=self.device, dtype=torch.float32).div_(255)
      next_images = torch.tensor(all_images[:, self.n:self.n + self.history], device=self.device, dtype=torch.float32).div_(255)
      ...
      states_sensors = torch.tensor(np.ascontiguousarray(all_sensors[:, self.history - 1]), device=self.device, dtype=torch.float32)
      next_sensors = torch.tensor(np.ascontiguousarray(all_sensors[:, self.n + self.history - 1]), device=self.device, dtype=torch.float32)
  ...
  def __next__(self):
    ...
    if self.image_only:
      images = torch.tensor(transitions['image'], dtype=torch.float32, device=self.device).div_(255)
      state = {"image": images}
    else:
      images = torch.tensor(transitions['image'], dtype=torch.float32, device=self.device).div_(255)
      sensors = torch.tensor(np.ascontiguousarray(transitions['sensor']), dtype=torch.float32, device=self.device)
