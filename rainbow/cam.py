...
class GradCAM:
    def __init__(self, model, use_deep_conv):
        ...
        layer.register_full_backward_hook(lambda m, grad_in, grad_out: self.gradients.append(grad_out[0]))

    def __call__(self, input_dict, args):
        # Forward
        output = self.model(input_dict)  # [1, A, atoms]
        support = torch.linspace(args.V_min, args.V_max, args.atoms, device=output.device)
